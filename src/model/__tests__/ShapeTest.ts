...
screen.blockCell(4,4).blockCell(4,5)
  .blockCell(5,4).blockCell(5,5);

plus.allowCell(1,0)
  .allowCell(0,1).allowCell(1,1).allowCell(2,1)
  .allowCell(1,2);

cross.blockCell(1,0).blockCell(1,2)
  .blockCell(0,1).blockCell(2,1);
  ...
  it('hasCell and getArea return same value', () => {
    let sum = (s: Shape) =>
      Array.from(Array(12).keys()).reduce(
        (c, x) => Array.from(Array(12).keys()).reduce(
